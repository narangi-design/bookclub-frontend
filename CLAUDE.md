...
[LoginPage](src/pages/LoginPage.tsx) offers two ways in, both ending in the same `login()` call: Telegram (primary) and the original username/password form (fallback, posts to `/api/auth/login`) — the backend keeps both live in parallel for now. `AuthUser.auth_method` (`'password' | 'telegram'`) tracks which one issued the session; [AccountModal](src/components/layout/AccountModal.tsx) uses it to hide the password-change form for Telegram sessions, since there's no password to change.

Telegram login has two paths into the same `/api/auth/telegram-login` flow, both in `LoginPage.tsx`, switched between automatically — the user only ever sees one button, never told there are two mechanisms:
- **Primary — plain redirect**: the visible button's `onClick` (`handleTelegramRedirectClick`) sets a `sessionStorage` flag, then `href={telegramAuthUrl()}` navigates straight to `oauth.telegram.org/auth` with `return_to` pointing back at `/login`; no popup, no `postMessage`. On return, an effect checks both `location.search` and `location.hash` for the signed fields (both instant-auto-confirm and manual/logged-out requests turned out to use the same query format — kept the fragment check anyway as cheap insurance) and immediately strips them via `history.replaceState`.
- **Fallback — Login Widget iframe** (`telegram-widget.js`, `data-onauth` → `window.onTelegramAuth`): `useFallback` state flips to `true` when the redirect-return effect finds the `sessionStorage` flag set (we did navigate away) but no signed fields came back — at that point the widget silently replaces the redirect link in the same spot, no "try this instead" copy shown. Has its own known gap on some users' VPNs, where the popup+`postMessage` handshake can silently fail (flash and close, no callback, no error) — mitigated by a `window.blur`/`focus` heuristic (popup opens → closes → callback still hasn't fired shortly after → show a "check your connection" hint), not fully solved. Don't remove this fallback without confirming the redirect path holds up for everyone first — it's exactly the failure mode the redirect path was added to avoid.
